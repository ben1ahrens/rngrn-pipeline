...
| Does anything currently constrain `D`? | **No penalty, bound or prior in the objective** — `D = exp(θ_D)`, `θ_D ~ N(0, 0.5²)` (`model.py:75,107`); `D` is shaped only indirectly, through `σ(k)` in the anchor and the hinges. **Updated 2026-08-11:** the *init* side is no longer untouched. Unit B4 (`e5bea3a`) added an opt-in, L-free init shift `θ_D ← θ_D − 2·log(kstar_obs)` (`model.py:179-186`, wired via `recover.py:456,462`), firewall-legal because `kstar_obs` comes from an FFT of the observed frame. It is **off by default** (`ModelConfig.d_init_from_kstar = False`, `config.py:84`), so the "No" above still holds for the default configuration — but §4.4's attachment point #1 is implemented, not future work. |

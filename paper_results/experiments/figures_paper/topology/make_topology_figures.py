...
# Collected into paper_results/ (branch feature/paper-results): run data lives
# under the pack root (WORKTREE == MAIN == PACK); dataset payloads and src/ come
# from the enclosing repo checkout. Original roots: the feature/lift-ladder
# worktree (WORKTREE) and the main checkout (MAIN).
PACK = HERE.parents[2]        # .../paper_results
REPO = PACK.parent            # repo root: data/datasets payload symlinks, src/
WORKTREE = PACK
MAIN = PACK

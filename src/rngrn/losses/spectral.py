...
def build_frame_targets(frame: np.ndarray, L: float, kstar_obs: float,
    ...
    raps_band_target = (band_power / band_sum).detach()

    block_target = spectral_block_torch(ch0, cfg.nblk).detach()
    ...
    return dict(k_centers=k_centers, kstar_obs=float(kstar_obs), L=float(L),
                raps_band_target=raps_band_target, block_target=block_target,

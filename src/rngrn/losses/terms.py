...
def kstar_anchor_si(model, xstar, kgrid, kstar_obs, tau=0.12, temp=60.0, eps=1e-12,
    ...
    `J`/`idx`: the hoisted Jacobian and the precomputed bracketing index, exactly as on
    `kstar_anchor` (R3 integration of the gpu-optim hoist). None for either reproduces the
    per-step form.
    """
    sig = model.dispersion(xstar, kgrid, J=J)
    sigbar = sig / (sig.max() - sig.min() + eps)
    lse = torch.logsumexp(sigbar * temp, dim=0) / temp
    sigbar_obs = _sigma_at(sigbar, kgrid, kstar_obs, idx=idx)
...
def kstar_anchor_si_batched(model, xstar, kgrid, kstar_obs, tau=0.12, temp=60.0, eps=1e-12,
    ...
    `J`/`idx`: the hoisted Jacobian and the precomputed bracketing index, exactly as on
    `kstar_anchor_batched` (R3 integration of the gpu-optim hoist). None for either
    reproduces the per-step form."""
    sig = model.dispersion(xstar, kgrid, J=J)                 # (B,K)
...
# pins it), so every existing caller of `terms.DEFAULT_WEIGHTS` is unaffected.
from . import term_registry as _term_registry  # noqa: E402  (after the terms it registers)
DEFAULT_WEIGHTS = _term_registry.default_weights()
